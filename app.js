...
app.post('/register/patient', function(req, res){
    ...
    var par = [req.body.id, req.body.password, req.body.name, req.body.age, req.body.contact];
    connection.query(sql, par, function(err, rows, fields){
        if(err){
            console.log(err);
            res.status(500);
...
app.post('/search', function(req, res){
    ...
    connection.query(sql, par, function(err, rows, fields){
        if(err){
            ...
            sql = 'select hospital_id, receipt_time, symptom, diagnosis_name, med_tr_name, pill_name from (select hospital_id, receipt_time, symptom, diagnosis_name, med_tr_name, pill_code from (select hospital_id, receipt_time, symptom, C.diagnosis_code, diagnosis_name, med_tr_name from (select hospital_id, receipt_time, symptom, B.diagnosis_code, diagnosis_name, med_tr_code from (select hospital_id, receipt_time, symptom, A.diagnosis_code, diagnosis_name from (select hospital_id, receipt_time, symptom, diagnosis_code from patient join medical_chart on patient.patient_id = medical_chart.patient_id and patient.patient_id = ?) AS A JOIN diagnosis on A.diagnosis_code = diagnosis.diagnosis_code) AS B JOIN medical_tr_guide on B.diagnosis_code = medical_tr_guide.diagnosis_code) AS C JOIN medical_tr on C.med_tr_code = medical_tr.med_tr_code) AS D JOIN prescription_guide ON D.diagnosis_code = prescription_guide.diagnosis_code) AS E JOIN pill on pill.pill_code = E.pill_code ORDER BY receipt_time DESC';
            par = [req.body.patient_id];
            connection.query(sql, par, function(err, results){
                res.render('view_patient.ejs', {id: req.body.patient_id, name: rows[0].patient_name, contact: rows[0].contact, age: rows[0].age, session: true, results: results});
            })
...
app.get('/view_receipt', function(req, res){
    ...
    connection.query(sql, [req.session.nickname], function(err, result){
        ...
        connection.query(sql, [req.session.nickname], function(err, results){
            res.render('view_receipt.ejs', {patient: result[0], results: results});
        })
